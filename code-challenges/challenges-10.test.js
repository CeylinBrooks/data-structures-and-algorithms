...
const count = (target, input) => {
  return input.reduce((accumulator, currentValue) => {
    const fives = currentValue.filter((value) => {
      return value === target; 
    });
    const numberOfFives = fives.length; 
    return accumulator += numberOfFives;
  }, 0);
};
...
const totalSum = (input) => {
  return input.reduce((accum, curr) => {
    accum+= curr.reduce((accum2, curr2) => {
      accum2 += curr2;
      return accum2;
    }, 0);
    return accum;
...
const divisibleByFiveTwoToThePower = (input) => {
  let result = [];
  input.filter(value =>{
    result.push(value.filter(newVal => {
      return newVal % 5 === 0 && typeof(newVal) === 'number';
    }))
  })
  return result.map(value =>{
    return value.map(newVal =>{
      return 2**newVal;
    })
  })
};
...
let findMaleAndFemale = (data) => {
  return data.filter(people => {
    return people.gender === 'female' || people.gender === 'male';
  }).map(character => {
...
let findShortest = (data) => {
  ...
  return data.sort(compare)[0].name;
};
